...
class TagMetaModelSynonyms extends JModelList
{
  ...
  protected function getListQuery()
  {
    ...
    if (!empty($search)) {
      $query->where('LOWER(s.keywords) LIKE '.$db->Quote('%'.$db->getEscaped($search, true).'%').'OR LOWER(a.synonyms) LIKE '.$db->Quote('%'.$db->getEscaped($search, true).'%')); // Search in keywords and synonyms only
    }
