				     ...
				     $keys = $title . ' ' . $thecontent;
				      ...
				      // work on the article
							$keywords = $this->seogeneratekeywords($keys, $blackList, $count, $minLength);
								...
								if (isset($metakey)) {      // check for existing field metakey in the mysql-table e.g. if new banner is saved in backend
						      if(strlen($metakey) == 0)
						      $metakey = $keywords;
								}

								if (isset($metadesc)) {      // check for existing field metadesc in the mysql-table e.g. if new banner is saved in backend
